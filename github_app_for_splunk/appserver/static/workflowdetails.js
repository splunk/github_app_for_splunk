   ...
   TableView
    ...
    var CustomRowRenderer = TableView.BaseRowExpansionRenderer.extend({
        ...
        this._TableView.on("click", function(e) {
            e.preventDefault();
            console.log(e);
            window.open("/app/github_app_for_splunk/build_details?form.workflow_id="+workflowIDCell.value+"&form.repoName="+repoNameCell.value+"&form.workflowName="+workflowName.value+"&form.field1.earliest=-24h%40h&form.field1.latest=now&form.timeRange.earliest=-30d%40d&form.timeRange.latest=now&form.workflowCount=25",'_self');
        });
    ...
    mycustomrowtable.on("click", function(e) {
       e.preventDefault();
       console.log(e.data);
       window.open("/app/github_app_for_splunk/build_details?form.repoName="+e.data["row.repository.full_name"]+"&form.workflowName="+e.data["row.workflow_job.name"]+"&form.field1.earliest=-24h%40h&form.field1.latest=now&form.timeRange.earliest=-30d%40d&form.timeRange.latest=now&form.workflowCount=25",'_blank');
    });
